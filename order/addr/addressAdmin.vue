...
<script>
	export default {
		...
		onLoad(option) {
			if (option.type) this.pageType = option.type
		},
		methods: {
			// 选择收货地址
			addrSel(item) {
				if (this.pageType == 1) {
					...
					uni.$emit('addrSelOrderSure', item)
			...
			addrUpdate(val) {
				console.log(val);
				uni.navigateTo({
					url: "addressUpdate?addrData=" + encodeURIComponent(JSON.stringify(val)),
				});
			},
			// 获取地址列表
			getAddrList() {
				...
				uni.request({
					url: this.$baseUrl + "/recvAddressList",
					header: {
