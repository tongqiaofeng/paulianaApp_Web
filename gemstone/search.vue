<template>
	...
	<view class="container" v-if="haveData">
		...
		<scroll-view
			scroll-y
			class="goods-scroll"
			:style="{ top: goodsStyle.top + 'rpx', height: goodsStyle.height }"
			...
			<view class="popup-content">
				...
				<view class="gemstone-list">
					<text
						...
						:class="{ active: gem == currentGemstone }"
						v-for="(gem, index) in gemstoneList"
						:key="index"
						@click="chooseGemstone(gem)"
						>{{ gem }}</text
					>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		async getGemstoneList() {
			const { data: res } = await Api.gemSortList();
			if (res.code == 200) {
		...
		async getSearchResult() {
			...
			const { data: res } = await Api.gemSearch(params);
		...
		confirmSearch() {
			if (this.keywords) {
				...
				this.timer = setTimeout(() => {
					this.searchList = [];
					this.loadStatus = false;
					this.currentGemstone = this.keywords;
					this.getSearchResult();
				}, 500);
			}
		},
