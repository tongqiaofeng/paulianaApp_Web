		...
		<view v-else class="list">
			<view v-for="(item, index) in list" :key="index" class="item row vCenter" @click="goToDetail(item.id)">
				...
				<view class="desc">
					...
					<view class="line">
						<text>编&emsp;号：</text> <text>{{item.productNumber}}</text>
					</view>
					<view class="line" @longpress="setCopyData(getPrice(item.unitPrice, item.currency, '/', 0))">
						<text>加工费：</text> <text>{{getPrice(item.processCost, 'CNY', '/', 0)}}</text>
					</view>
					<view class="line" @longpress="setCopyData(getPrice(item.unitPrice, item.currency, '/', 0))">
						<text>成本港币：</text> <text>{{getPrice(item.totalHkPrice, 'HKD', '/', 0)}}</text>
					</view>
					<view class="line" @longpress="setCopyData(getPrice(item.unitPrice, item.currency, '/', 0))">
						<text>成本人民币：</text> <text>{{getPrice(item.totalCnPrice, 'CNY', '/', 0)}}</text>
					</view>
					<view class="line" @longpress="setCopyData(getPrice(item.totalPrice, item.currency, '/', 0))">
						<text>标签价：</text> <text>{{getPrice(item.tagPrice, 'CNY', '/', 0)}}</text>
					</view>
					<view class="line">
	...
	export default {
		data() {
			return {
				...
				stateId:0,
				stateList:[ { value: -1, text: "全部" }, { value: 0, text: "库存中" }, { value: 3, text: "已出售" } ],
		...
		methods: {
			search(rest) {
				...
				uni.request({
					url: this.$baseMagageUrl + "/productList",
					method: "POST",
					header: {
			...
			getStockList(){
				uni.request({
					url: this.$baseMagageUrl + "/warehouseProductTotalGet",
					method: "POST",
					header: {
						"content-type": "application/json",
						token: uni.getStorageSync("managerToken"),
			...
			goToDetail(id){
				uni.navigateTo({
					url: '../pauliana/jewelryDetails?id=' + id,
				})
			}
		}
	}
