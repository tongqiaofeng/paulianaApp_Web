		...
		<view v-else class="inventory-main">
			...
			<view class="plantList">
				<view v-for="(item, index) in list" :key="index" class="plant">
                    ...
                    ">
										<view>
											<view class="cs" v-if="item.unitPrice" :style="{
                            ...
                            : item.currency +
                              ...
                              formatNumberRgx(
                                parseInt(item.unitPrice * item.weight)
											...
											<view class="cs" v-if="item.weight > 0" :style="{
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			if (this.list.length == 0 && uni.getStorageSync("managerToken").length > 0) {
				console.log("重新获取数据");
				this.getStockList();
		...
		methods: {
			...
			search() {
				...
				if (this.stockIdx > 0) 
					data.storageIdList = [this.stocks[this.stockIdx].id];
				if (this.materialIdx > 0)
					data.materialId = this.materialModelList[this.materialIdx].id;
				if (this.stateIdx > 0) 
				data.state = this.states[this.stateIdx].id;

				uni.request({
					url: this.$baseMagageUrl + "/jewellery/api/stockMaterialList",
					method: "POST",
					header: {
			...
			getStockList() {
				uni.request({
					url: this.$baseMagageUrl + "/jewellery/api/warehouseFactoryList",
					method: "POST",
					header: {
						"content-type": "application/json",
						token: uni.getStorageSync("managerToken"),
			...
			getMaterialModelList() {
				uni.request({
					url: this.$baseMagageUrl + "/jewellery/api/materialModelList",
					header: {
						"content-type": "application/json",
						token: uni.getStorageSync("managerToken"),
					},
